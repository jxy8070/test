#include "stable.h"
#include "device_spk_2.h"
#include "protocal/packetbase.h"
#include "protocal/packet.h"
#include "device/devicemanager.h"
#include "deviceSetting/alarmconfig.h"

Device_Spk_2::Device_Spk_2(const DanteDevice *danteDev)
    :DanteDevice(danteDev)
{
    _type = DEV_TYPE_DNTE_SPK_2;
...
Device_Spk_2::Device_Spk_2(QString mac, DevType devType, QString customName, QString serial)
...
void Device_Spk_2::saveAlarmSetting()
{
    _alarmCfg->saveAlarmSetting();
}

void Device_Spk_2::loadAlarmSetting()
{
    _alarmCfg->loadAlarmSetting();
}

void Device_Spk_2::clearAlarmSetting()
{
    _alarmCfg->clearAlarmSetting();
}

void Device_Spk_2::processPacket(PacketBase* pack)
{
#ifdef QT_DEBUG
//    if(pack->getMsgType() != 126)
//        qDebug()<<"Device_Spk_2 receive Pack:" <<pack->getMsgType();
#endif
    ...
    case MSG_TYPE_ADC_VALUE:
    {
        if (!customName().isEmpty())
        {
            ...
            _alarmCfg->checkLimit(DEV_TYPE_DNTE_SPK_2, _adcData);
...
void Device_Spk_2::sendInitPack()
{
    sendPacket_Nobody(MSG_TYPE_DEV_INIT);
}

void Device_Spk_2::sendHeartbeatPack()
{
    sendPacket_Nobody(MSG_TYPE_HEARTBEAT);
}

void Device_Spk_2::sendAllConfigPack(bool isNeedDisSuccess)
{
    MsgData_Spk cmmData;
...
void Device_Spk_2::sendVoltTypePack(quint8 voltType)
{
...
void Device_Spk_2::sendSwitch(bool flag)
{
    _configData.Switch = flag;
    sendCommonDataPack();
}

void Device_Spk_2::sendVolume(qint8 vol)
{
...
void Device_Spk_2::sendMute(bool flag)
{
...
void Device_Spk_2::sendPhase(bool flag)
{
...
void Device_Spk_2::sendRouter(quint8 routerCh)
{
...
void Device_Spk_2::sendCommonDataPack()
{
    MsgData_Spk cmmData;
...
//发送控制信号
void Device_Spk_2::sendEqPack(quint8 ch, bool bypass, EqData* eqPara)
{
    Q_ASSERT(ch == 1);
...
void Device_Spk_2::sendDynPack(quint8 ch, DynData& dynPara)
{
    Q_ASSERT(ch == 1);
...
void Device_Spk_2::sendDividerPack(quint8 ch, DividerData& hf, DividerData& lf )
{
    Q_ASSERT(ch == 1);
...
void Device_Spk_2::sendSave2DevicePack()
{
    _saveState = DevSave_Save2Dev;
    sendPacket_Nobody(MSG_TYPE_SAVE);
}

void Device_Spk_2::sendUpdatePackage(quint32 totalCount, quint32 offset, quint32 paraLen, QByteArray buff)
{
    QByteArray packBuff;
...
// 另存场景为文件
void Device_Spk_2::serialize(QString fileName)
{
    QFile f(fileName);
...
void Device_Spk_2::deserialize(QString fileName)
{
    QFile file(fileName);
...
void Device_Spk_2::initData(const ConfigData_Spk& configData)
{
    if (_factoryMode)
    {
...
QString Device_Spk_2::getRouterInfo()
{
    QString str;
