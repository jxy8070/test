#include "devsetting_amp_module.h"
#include "ui_devsetting_amp_module.h"
...
DevSetting_Amp_Module::DevSetting_Amp_Module(DeviceItem_RT1052_AMP_MODULE_4_4 *devItem, QWidget *parent) :
    DevSettingDlg(parent),
    ui(new Ui::DevSetting_Amp_Module)
{
    ui->setupUi(this);
    _devItem = devItem;
...
DevSetting_Amp_Module::~DevSetting_Amp_Module()
{
    _devAmp->sendStopMoniter();
    delete ui;
}

void DevSetting_Amp_Module::DisplaySlot(struct MsgData_RT1052_Poe b)
{
    //电平表
...
void DevSetting_Amp_Module::connectSigSlots()
{
    /* 从设备初始化界面 */
...
void DevSetting_Amp_Module::displayConfigValue()
{
...
/* Gain\Mute\Phase\Delay */
void DevSetting_Amp_Module::setGain(int value)
{
//    qDebug()<<"gain:"<<value;
...
void DevSetting_Amp_Module::setMute(bool enable)
{
//    qDebug()<<"mute:"<<enable;
...
void DevSetting_Amp_Module::setPhase(bool enable)
{
...
void DevSetting_Amp_Module::setDelay(int value)
{
    quint16 remain = MAX_DELAY_TIME
            - ui->AIn1_Delay->value()
            - ui->AIn2_Delay->value()
...
/* Eq */
void DevSetting_Amp_Module::setEq()
{
//    qDebug()<<"eq";
    int channel;
    EqData4B eq4B;
...
/* 处理函数 */
void DevSetting_Amp_Module::onChannelEqChanged(quint8 ch, bool bypass, EqData *eqPara)
{
    _devAmp->sendEq(0, ch, bypass, eqPara);
}

void DevSetting_Amp_Module::onEqChanged(quint8 ch, bool bypass, EqData *eqPara)
{
    _devAmp->sendEq(1, ch, bypass, eqPara);
}

/* Dyn */
void DevSetting_Amp_Module::setDyn()
{
    int channel;
    DynData dynData;
...
/* 处理函数 */
void DevSetting_Amp_Module::onDynChanged(quint8 ch, bool isIn ,DynData dynData)
{
    _devAmp->sendDyn(ch, dynData);
}

/* XOVER */
void DevSetting_Amp_Module::setXOver()
{
    quint8 ch = 0;
...
/* AOut Mixer */
void DevSetting_Amp_Module::setAOutMixer()
{
...
void DevSetting_Amp_Module::on_save_clicked()
{
    _devAmp->sendSave();
}
