#include "devsetting_amp_quad_fangtu.h"
#include "ui_devsetting_amp_quad_fangtu.h"
...
// 构造函数
DevSetting_Amp_Quad_FangTu::DevSetting_Amp_Quad_FangTu(DeviceItem_Amp_Quad *devItem,  QWidget *parent) :
    DevSettingDlg(parent),
    ui(new Ui::DevSetting_Amp_Quad_FangTu)
{
    ui->setupUi(this);
    _devItem = devItem;
...
//    ui->AIn4_Delay->setRange(0, MAX_DELAY_TIME);
    ...
    ui->AIn1_delay_ms->setRange(0, MAX_DELAY_TIME / 48.0);
    ui->AIn2_delay_ms->setRange(0, MAX_DELAY_TIME / 48.0);
    ui->AIn3_delay_ms->setRange(0, MAX_DELAY_TIME / 48.0);
    ui->AIn4_delay_ms->setRange(0, MAX_DELAY_TIME / 48.0);
...
DevSetting_Amp_Quad_FangTu::~DevSetting_Amp_Quad_FangTu()
{
    _devAmp->sendStopMonitor();
    delete ui;
}

void DevSetting_Amp_Quad_FangTu::connectSigSlots()
{
    /* 从设备初始化界面 */
...
//    connect(ui->AIn4_Delay, SIGNAL(valueChanged(int)), this, SLOT(setDelay(int)));
    ...
    connect(ui->AIn1_delay_ms, QOverload<double>::of(&ConfigGainWidget::valueChanged), this, &DevSetting_Amp_Quad_FangTu::setDelayMs);
    connect(ui->AIn2_delay_ms, QOverload<double>::of(&ConfigGainWidget::valueChanged), this, &DevSetting_Amp_Quad_FangTu::setDelayMs);
    connect(ui->AIn3_delay_ms, QOverload<double>::of(&ConfigGainWidget::valueChanged), this, &DevSetting_Amp_Quad_FangTu::setDelayMs);
    connect(ui->AIn4_delay_ms, QOverload<double>::of(&ConfigGainWidget::valueChanged), this, &DevSetting_Amp_Quad_FangTu::setDelayMs);
...
void DevSetting_Amp_Quad_FangTu::displayConfigValue()
{
...
//    ui->AIn4_Delay->setValue(cfgData.Aout_delay[3].delay);
    ...
    quint16 remain = MAX_DELAY_TIME / 48.0
             - ui->AIn1_delay_ms->value()
             - ui->AIn2_delay_ms->value()
             - ui->AIn3_delay_ms->value()
             - ui->AIn4_delay_ms->value();//最大延时为260ms,调整为一个样值一调整
    ui->AIn1_delay_ms->setRange(0, ui->AIn1_delay_ms->value() + remain);
    ui->AIn2_delay_ms->setRange(0, ui->AIn2_delay_ms->value() + remain);
    ui->AIn3_delay_ms->setRange(0, ui->AIn3_delay_ms->value() + remain);
    ui->AIn4_delay_ms->setRange(0, ui->AIn4_delay_ms->value() + remain);
}

/* Gain\Mute\Phase\Delay */
void DevSetting_Amp_Quad_FangTu::setGain(int value)
{
...
void DevSetting_Amp_Quad_FangTu::setMute(bool enable)
{
...
void DevSetting_Amp_Quad_FangTu::setPhase(bool enable)
{
...
//void DevSetting_Amp_Quad_FangTu::setDelay(int value)
//{
...
//        ui->AIn1_delay_ms->disconnect();
//        ui->AIn1_delay_ms->setValue(value / 48.0);
//        connect(ui->AIn1_delay_ms, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &DevSetting_Amp_Quad_FangTu::setDelayMs);
//    }
//    else if (sender() == ui->AIn2_Delay)
//    {
//        _devAmp->sendDelay(1, value);

//        ui->AIn2_delay_ms->disconnect();
//        ui->AIn2_delay_ms->setValue(value / 48.0);
//        connect(ui->AIn2_delay_ms, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &DevSetting_Amp_Quad_FangTu::setDelayMs);
//    }
//    else if (sender() == ui->AIn3_Delay)
//    {
//        _devAmp->sendDelay(2, value);

//        ui->AIn3_delay_ms->disconnect();
//        ui->AIn3_delay_ms->setValue(value / 48.0);
//        connect(ui->AIn3_delay_ms, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &DevSetting_Amp_Quad_FangTu::setDelayMs);
//    }
//    else if (sender() == ui->AIn4_Delay)
//    {
//        _devAmp->sendDelay(3, value);

//        ui->AIn4_delay_ms->disconnect();
//        ui->AIn4_delay_ms->setValue(value / 48.0);
//        connect(ui->AIn4_delay_ms, QOverload<double>::of(&QDoubleSpinBox::valueChanged), this, &DevSetting_Amp_Quad_FangTu::setDelayMs);
//    }
//}

void DevSetting_Amp_Quad_FangTu::setDelayMs(double value)
{
    quint16 remain = MAX_DELAY_TIME / 48.0
            - ui->AIn1_delay_ms->value()
            - ui->AIn2_delay_ms->value()
            - ui->AIn3_delay_ms->value()
            - ui->AIn4_delay_ms->value();
    ...
    if(sender() == ui->AIn1_delay_ms) {
            _devAmp->sendDelay(0, value * 48);
    }
    else if(sender() == ui->AIn2_delay_ms) {
            _devAmp->sendDelay(1, value * 48);
    }
    else if(sender() == ui->AIn3_delay_ms) {
            _devAmp->sendDelay(2, value * 48);
    }
    else if(sender() == ui->AIn4_delay_ms) {
            _devAmp->sendDelay(3, value * 48);
    }
}

/* Eq */
void DevSetting_Amp_Quad_FangTu::setEq()
{
    int channel;
    EqData4B eq4B;
    uint8_t type = 0;
...
/* 处理函数 */
void DevSetting_Amp_Quad_FangTu::onChannelEqChanged(quint8 ch, bool bypass, EqData *eqPara)
{
    _devAmp->sendEq(0, ch, bypass, eqPara);
}

void DevSetting_Amp_Quad_FangTu::onEqChanged(quint8 ch, bool bypass, EqData *eqPara)
{
    _devAmp->sendEq(1, ch, bypass, eqPara);
}

/* Dyn */
void DevSetting_Amp_Quad_FangTu::setDyn()
{
    int channel;
    DynData dynData;
...
/* 处理函数 */
void DevSetting_Amp_Quad_FangTu::onDynChanged(quint8 ch, bool isIn ,DynData dynData)
{
    _devAmp->sendDyn(ch, dynData);
}

/* XOVER */
void DevSetting_Amp_Quad_FangTu::setXOver()
{
    quint8 ch = 0;
...
/* AOut Mixer */
void DevSetting_Amp_Quad_FangTu::setAOutMixer()
{
...
/* NOut Route */
void DevSetting_Amp_Quad_FangTu::setNOutRoute()
{
...
void DevSetting_Amp_Quad_FangTu::DisplaySlot(struct MsgData_RT1052_Poe b)
{
...
//// 另存场景为文件
//void DevSetting_Amp_Quad_FangTu::on_saveAsBtn_clicked()
//{
...
//// 加载场景文件
//void DevSetting_Amp_Quad_FangTu::on_loadBtn_clicked()
//{
 ...
 *
 *****************************************************************************************/
//void DevSetting_Amp_Quad_FangTu::on_router_clicked()
//{
 ...
 *****************************************************************************************/
///* 第一路 */
//void DevSetting_Amp_Quad_FangTu::on_filter1HPF_clicked()
//{
...
//void DevSetting_Amp_Quad_FangTu::on_filter1Bypass_clicked()
//{
...
//void DevSetting_Amp_Quad_FangTu::on_filter1LPF_clicked()
//{
//    _ui->filter1HPFSet->setEnabled(false);
...
//void DevSetting_Amp_Quad_FangTu::on_filter1HPFSet_clicked()
//{
...
//void DevSetting_Amp_Quad_FangTu::on_filter1LPFSet_clicked()
//{
...
/* 处理函数 */
void DevSetting_Amp_Quad_FangTu::onPfDataChanged(quint8 channel)
{
...
void DevSetting_Amp_Quad_FangTu::onPfDataChanged(quint8 ch, bool hfb, bool lfb)
{
//    /* 数据同步时不再次发送 */
//    if (_syncing)
//    {
...
void DevSetting_Amp_Quad_FangTu::on_save_clicked()
{
    _devAmp->sendSave();
}
