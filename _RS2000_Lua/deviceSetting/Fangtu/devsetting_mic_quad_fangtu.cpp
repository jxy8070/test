#include "devsetting_mic_quad_fangtu.h"
#include "ui_devsetting_mic_quad_fangtu.h"
 ...
 * */
// 构造函数
DevSetting_Mic_Quad_FangTu::DevSetting_Mic_Quad_FangTu(DeviceItem_Mic_Quad *devItem,  QWidget *parent) :
    DevSettingDlg(parent),
    ui(new Ui::DevSetting_Mic_Quad_FangTu)
{
    ui->setupUi(this);
    _devItem = devItem;
...
DevSetting_Mic_Quad_FangTu::~DevSetting_Mic_Quad_FangTu()
{
    _dev->sendStopMonitor();
    delete ui;
}

void DevSetting_Mic_Quad_FangTu::connectSigSlots()
{
    /* 从设备初始化界面 */
...
void DevSetting_Mic_Quad_FangTu::displayConfigValue()
{
...
void DevSetting_Mic_Quad_FangTu::MicGainChanged(int value)
{
    int ch = 0;
...
void DevSetting_Mic_Quad_FangTu::MicPhantomChanged(bool enable)
{
    int ch = 0;
...
void DevSetting_Mic_Quad_FangTu::GainChanged(int value)
{
    int ch = 0;
    if (sender() == ui->gain_1)
    {
...
void DevSetting_Mic_Quad_FangTu::setMute(bool enable)
{
...
void DevSetting_Mic_Quad_FangTu::xoverClicked()
{
    int ch = 0;
...
void DevSetting_Mic_Quad_FangTu::DynClicked()
{
    int channel;
...
void DevSetting_Mic_Quad_FangTu::onDynChanged(quint8 ch, bool isIn ,DynData dynData)
{
    _dev->sendDyn(ch, dynData);
}

void DevSetting_Mic_Quad_FangTu::EqClicked()
{
    int channel;
...
void DevSetting_Mic_Quad_FangTu::onEqChanged(quint8 ch, bool bypass, EqData *eqPara)
{
    _dev->sendEq(ch, bypass, eqPara);
}

void DevSetting_Mic_Quad_FangTu::OutPhaseClicked(bool enable)
{
    int ch = 0;
...
void DevSetting_Mic_Quad_FangTu::MixerClicked()
{
    if (sender() == ui->Mixer)
    {
        QStringList in, out;
...
void DevSetting_Mic_Quad_FangTu::MixerChangedSlot(int out, int in, bool enable)
{
    _dev->sendMixer(out, in, enable);
}

void DevSetting_Mic_Quad_FangTu::OutGainChanged(int value)
{
    int ch = 0;
...
void DevSetting_Mic_Quad_FangTu::DisplaySlot(MsgData_Mic_Quad b)
{
...
void DevSetting_Mic_Quad_FangTu::on_save_clicked()
{
    _dev->sendSave();
}
