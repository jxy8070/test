...
// 构造函数
DevSetting_Spk_1::DevSetting_Spk_1(DeviceItem_Spk *devItem, QWidget *parent)
    :QDialog(parent),
    _ui(new Ui::DevSetting_Spk)
{
    ...
    _devSpk =qobject_cast<Device_Spk_1*>(devItem->getDanteDevice());
...
void DevSetting_Spk_1::closeSelf()
{
    if (!_hasData)
    {
        close();
...
DevSetting_Spk_1::~DevSetting_Spk_1()
{
    saveAlarmSetting();
...
void DevSetting_Spk_1::keyPressEvent(QKeyEvent *e)
{
  switch (e->key()) {
  case Qt::Key_Enter:
      break;
...
void DevSetting_Spk_1::factoryModeReInit()
{
...
// 显示设备发来的电压电流温度等数据
void DevSetting_Spk_1::onAdcDataChanged(AlarmConfig *alarmCfg)
{
...
void DevSetting_Spk_1::connectSigSlots()
{
    /* 第一路延时选择 */
...
void DevSetting_Spk_1::setAlarmMode(bool b)
{
    _ui->groupBox->setEnabled(b);
    _devSpk->getAlarmCfg()->setAlarmEnable(b);
}

void DevSetting_Spk_1::on_router_clicked()
{
    SpkRouter_1 router(_devSpk, this);
    router.exec();
}

void DevSetting_Spk_1::on_alarmButton_clicked()
{
    AlarmRecord alarmFrm(_devSpk->getMac());
    alarmFrm.exec();
}

void DevSetting_Spk_1::on_eq_clicked()
{
...
void DevSetting_Spk_1::on_dividerHF_clicked()
{
...
void DevSetting_Spk_1::on_dividerLF_clicked()
{
...
void DevSetting_Spk_1::onPfDataChanged(quint8 channel)
{
    Q_ASSERT(channel == 1);
...
void DevSetting_Spk_1::onPfDataChanged(quint8 ch, bool hfb, bool lfb)
{
    Q_ASSERT(ch == 1);
...
void DevSetting_Spk_1::on_dyn_clicked()
{
...
void DevSetting_Spk_1::onEqChanged(quint8 ch, bool bypass, EqData *eqPara)
{
    _devSpk->sendEqPack(ch, bypass, eqPara);
}

void DevSetting_Spk_1::onDynChanged(quint8 ch, bool isIn ,DynData dynData)
{
    Q_UNUSED(isIn)
    _devSpk->sendDynPack(ch, dynData);
}

void DevSetting_Spk_1::setLimiterValue()
{
...
void DevSetting_Spk_1::setIntValueDisplay(int i)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
...
void DevSetting_Spk_1::setEnableWidgets(int i)
{
    AlarmConfig *alarmCfg = _devSpk->getAlarmCfg();
...
void DevSetting_Spk_1::setSliderValue()
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
...
// 保存到设备
void DevSetting_Spk_1::save2Device()
{
    _devSpk->sendSave2DevicePack();
}

void DevSetting_Spk_1::displayConfigData()
{
    _syncing = true;
...
// 加载场景文件
void DevSetting_Spk_1::on_loadButton_clicked()
{
...
// 另存场景为文件
void DevSetting_Spk_1::on_saveAsButton_clicked()
{
...
void DevSetting_Spk_1::on_pushButton60v_clicked()
{
    resetVoltCurrLimit(0);
    _devSpk->sendVoltTypePack(0);
}

void DevSetting_Spk_1::on_pushButton100v_clicked()
{
...
void DevSetting_Spk_1::resetVoltCurrLimit(quint8 voltType)
{
...
void DevSetting_Spk_1::loadAlarmSetting()
{
    AlarmConfig *alarmCfg = _devSpk->getAlarmCfg();
...
void DevSetting_Spk_1::saveAlarmSetting()
{
   AlarmConfig *alarmCfg = _devSpk->getAlarmCfg();
   alarmCfg->saveAlarmSetting();
}

void DevSetting_Spk_1::on_gain1P_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
...
void DevSetting_Spk_1::on_gain1M_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
