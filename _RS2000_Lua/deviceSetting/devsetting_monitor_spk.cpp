#include "stable.h"
#include "devsetting_monitor_spk.h"
#include "ui_devsetting_monitor_spk.h"
#include "device/device_monitor_spk.h"
#include "deviceItem/deviceitem_monitor_spk.h"
 ...
 * */
// 构造函数
DevSetting_Monitor_SPK::DevSetting_Monitor_SPK(DeviceItem_Monitor_SPK *devItem,  QWidget *parent) :
    DevSettingDlg(parent),
    _ui(new Ui::DevSetting_Monitor_SPK)
{
    _ui->setupUi(this);
    _devItem = devItem;
    Q_ASSERT(devItem->getAES70Device());
    _devAmp = qobject_cast<Device_Monitor_SPK*>(devItem->getAES70Device());

    connect(_devAmp, SIGNAL(adcDataChanged(AlarmConfig*)), this, SLOT(onAdcDataChanged(AlarmConfig*)));
    setAttribute(Qt::WA_QuitOnClose, true);//使小部件接受closeEvent事件
    ...
    _ui->deviceName->setText(_devItem->getAES70Device()->getDeviceName());
    _ui->IPAddr->setText(_devAmp->getDeviceIp());
    _ui->MacAddrLabel->setText(_devAmp->getAES70Mac());
...
void DevSetting_Monitor_SPK::closeSelf()
{
    if (!_hasData)
    {
//        close();
...
DevSetting_Monitor_SPK::~DevSetting_Monitor_SPK()
{
    saveAlarmSetting();
...
// 显示设备发来的电压电流温度等数据
void DevSetting_Monitor_SPK::onAdcDataChanged(AlarmConfig *alarmCfg)
{
...
void DevSetting_Monitor_SPK::connectSigSlots()
{
    /* 报警相关 */
...
void DevSetting_Monitor_SPK::setEnableWidgets(int i)
{
...
void DevSetting_Monitor_SPK::setAlarmMode(bool b)
{
...
void DevSetting_Monitor_SPK::setLimiterValue()
{
    AlarmConfig *alarmCfg = _devAmp->getAlarmCfg();
...
// 保存到设备
void DevSetting_Monitor_SPK::on_saveBtn_clicked()
{
    _devAmp->sendSave2DevicePack();
}

void DevSetting_Monitor_SPK::displayConfigValue()
{
    _syncing = true;
    ConfigData_Monitor_SPK& cfgData = _devAmp->getConfigData();

    _ui->tab_3->setEnabled(true/*cfgData.Switch*/);

    _ui->delay1Slider->setValue(cfgData.CH[Device_Monitor_SPK::AMP_CH1].ControlValue.delay / 3);
    _ui->delay1Value->setText(QString::number(cfgData.CH[Device_Monitor_SPK::AMP_CH1].ControlValue.delay / 3));
    _ui->gain1Slider->setValue(cfgData.CH[Device_Monitor_SPK::AMP_CH1].ControlValue.volume);
    _ui->gain1Value->setText(QString::number(cfgData.CH[Device_Monitor_SPK::AMP_CH1].ControlValue.volume));
    _ui->gain1M->setChecked(cfgData.CH[Device_Monitor_SPK::AMP_CH1].ControlValue.mute);
    _ui->gain1P->setChecked(cfgData.CH[Device_Monitor_SPK::AMP_CH1].ControlValue.phase);

    _ui->delay2Slider->setValue(cfgData.CH[Device_Monitor_SPK::AMP_CH2].ControlValue.delay / 3);
    _ui->delay2Value->setText(QString::number(cfgData.CH[Device_Monitor_SPK::AMP_CH2].ControlValue.delay / 3));
    _ui->gain2Slider->setValue(cfgData.CH[Device_Monitor_SPK::AMP_CH2].ControlValue.volume);
    _ui->gain2Value->setText(QString::number(cfgData.CH[Device_Monitor_SPK::AMP_CH2].ControlValue.volume));
    _ui->gain2M->setChecked(cfgData.CH[Device_Monitor_SPK::AMP_CH2].ControlValue.mute);
    _ui->gain2P->setChecked(cfgData.CH[Device_Monitor_SPK::AMP_CH2].ControlValue.phase);

    _ui->delay3Slider->setValue(cfgData.CH[Device_Monitor_SPK::AMP_CH3].ControlValue.delay / 3);
    _ui->delay3Value->setText(QString::number(cfgData.CH[Device_Monitor_SPK::AMP_CH3].ControlValue.delay / 3));
    _ui->gain3Slider->setValue(cfgData.CH[Device_Monitor_SPK::AMP_CH3].ControlValue.volume);
    _ui->gain3Value->setText(QString::number(cfgData.CH[Device_Monitor_SPK::AMP_CH3].ControlValue.volume));
    _ui->gain3M->setChecked(cfgData.CH[Device_Monitor_SPK::AMP_CH3].ControlValue.mute);
    _ui->gain3P->setChecked(cfgData.CH[Device_Monitor_SPK::AMP_CH3].ControlValue.phase);

    _ui->delay4Slider->setValue(cfgData.CH[Device_Monitor_SPK::AMP_CH4].ControlValue.delay / 3);
    _ui->delay4Value->setText(QString::number(cfgData.CH[Device_Monitor_SPK::AMP_CH4].ControlValue.delay / 3));
    _ui->gain4Slider->setValue(cfgData.CH[Device_Monitor_SPK::AMP_CH4].ControlValue.volume);
    _ui->gain4Value->setText(QString::number(cfgData.CH[Device_Monitor_SPK::AMP_CH4].ControlValue.volume));
    _ui->gain4M->setChecked(cfgData.CH[Device_Monitor_SPK::AMP_CH4].ControlValue.mute);
    _ui->gain4P->setChecked(cfgData.CH[Device_Monitor_SPK::AMP_CH4].ControlValue.phase);

    if (cfgData.CH[Device_Monitor_SPK::AMP_CH1].divider.hf.bypass == 0x01 && cfgData.CH[Device_Monitor_SPK::AMP_CH1].divider.lf.bypass == 0x01)
    {
        _ui->filter1Bypass->setChecked(true);
        emit _ui->filter1Bypass->clicked(true);
    }
    else if (cfgData.CH[Device_Monitor_SPK::AMP_CH1].divider.hf.bypass == 0x01)
    {
        _ui->filter1LPF->setChecked(true);
        emit _ui->filter1LPF->clicked(true);
    }
    else if (cfgData.CH[Device_Monitor_SPK::AMP_CH1].divider.lf.bypass == 0x01)
    {
        _ui->filter1HPF->setChecked(true);
        emit _ui->filter1HPF->clicked(true);
    }

    if (cfgData.CH[Device_Monitor_SPK::AMP_CH2].divider.hf.bypass == 0x01 && cfgData.CH[Device_Monitor_SPK::AMP_CH1].divider.lf.bypass == 0x01)
    {
        _ui->filter2Bypass->setChecked(true);
        emit _ui->filter2Bypass->clicked(true);
    }
    else if (cfgData.CH[Device_Monitor_SPK::AMP_CH2].divider.hf.bypass == 0x01)
    {
        _ui->filter2LPF->setChecked(true);
        emit _ui->filter2LPF->clicked(true);
    }
    else if (cfgData.CH[Device_Monitor_SPK::AMP_CH2].divider.lf.bypass == 0x01)
    {
        _ui->filter2HPF->setChecked(true);
        emit _ui->filter2HPF->clicked(true);
    }

    if (cfgData.CH[Device_Monitor_SPK::AMP_CH3].divider.hf.bypass == 0x01 && cfgData.CH[Device_Monitor_SPK::AMP_CH3].divider.lf.bypass == 0x01)
    {
        _ui->filter3Bypass->setChecked(true);
        emit _ui->filter3Bypass->clicked(true);
    }
    else if (cfgData.CH[Device_Monitor_SPK::AMP_CH3].divider.hf.bypass == 0x01)
    {
        _ui->filter3LPF->setChecked(true);
        emit _ui->filter3LPF->clicked(true);
    }
    else if (cfgData.CH[Device_Monitor_SPK::AMP_CH3].divider.lf.bypass == 0x01)
    {
        _ui->filter3HPF->setChecked(true);
        emit _ui->filter3HPF->clicked(true);
    }

    if (cfgData.CH[Device_Monitor_SPK::AMP_CH4].divider.hf.bypass == 0x01 && cfgData.CH[Device_Monitor_SPK::AMP_CH4].divider.lf.bypass == 0x01)
    {
        _ui->filter4Bypass->setChecked(true);
        emit _ui->filter4Bypass->clicked(true);
    }
    else if (cfgData.CH[Device_Monitor_SPK::AMP_CH4].divider.hf.bypass == 0x01)
    {
        _ui->filter4LPF->setChecked(true);
        emit _ui->filter4LPF->clicked(true);
    }
    else if (cfgData.CH[Device_Monitor_SPK::AMP_CH4].divider.lf.bypass == 0x01)
    {
...
void DevSetting_Monitor_SPK::onShowAlarmDlg()
{
    AlarmRecord alarmFrm(_devAmp->getAES70Mac());
    alarmFrm.exec();
}

void DevSetting_Monitor_SPK::keyPressEvent(QKeyEvent *e)
{
    switch (e->key()) {
    case Qt::Key_Enter:
        break;
...
void DevSetting_Monitor_SPK::factoryModeReInit()
{
...
// 另存场景为文件
void DevSetting_Monitor_SPK::on_saveAsBtn_clicked()
{
...
// 加载场景文件
void DevSetting_Monitor_SPK::on_loadBtn_clicked()
{
...
void DevSetting_Monitor_SPK::on_pushButton60v_clicked()
{
    resetVoltCurrLimit(0);
    _devAmp->sendVoltTypePack(0);
}

void DevSetting_Monitor_SPK::on_pushButton100v_clicked()
{
...
void DevSetting_Monitor_SPK::resetVoltCurrLimit(quint8 voltType)
{
 ...
 *
 *****************************************************************************************/
void DevSetting_Monitor_SPK::on_gain1P_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
    ...
    _devAmp->sendPhase(Device_Monitor_SPK::AMP_CH1, checked);
}

void DevSetting_Monitor_SPK::on_gain1M_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
    ...
    _devAmp->sendMute(Device_Monitor_SPK::AMP_CH1, checked);
}

void DevSetting_Monitor_SPK::on_gain2P_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
    ...
    _devAmp->sendPhase(Device_Monitor_SPK::AMP_CH2, checked);
}

void DevSetting_Monitor_SPK::on_gain2M_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
    ...
    _devAmp->sendMute(Device_Monitor_SPK::AMP_CH2, checked);
}

void DevSetting_Monitor_SPK::on_gain3P_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
    ...
    _devAmp->sendPhase(Device_Monitor_SPK::AMP_CH3, checked);
}

void DevSetting_Monitor_SPK::on_gain3M_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
    ...
    _devAmp->sendMute(Device_Monitor_SPK::AMP_CH3, checked);
}

void DevSetting_Monitor_SPK::on_gain4P_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
    ...
    _devAmp->sendPhase(Device_Monitor_SPK::AMP_CH4, checked);
}

void DevSetting_Monitor_SPK::on_gain4M_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
    ...
    _devAmp->sendMute(Device_Monitor_SPK::AMP_CH4, checked);
}

/* 推子最后处理 */
void DevSetting_Monitor_SPK::setIntValueDisplay(int i)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
    ...
    if(sender() == _ui->delay1Slider)
    {
        ...
        cntvlu = &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH1].ControlValue;
        cntvlu->delay = i * 3;

        _devAmp->sendCommonDataPack(Device_Monitor_SPK::AMP_CH1, *cntvlu);
    }
    else if (sender() == _ui->gain1Slider)
    {
        _ui->gain1Value->setText(QString::number(i));

        _devAmp->sendVolume(Device_Monitor_SPK::AMP_CH1, i);
    }
    ...
    else if(sender() == _ui->delay2Slider)
    {
        ...
        cntvlu = &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH2].ControlValue;//.ch2_ControlValue;
        cntvlu->delay = i * 3;

        _devAmp->sendCommonDataPack(Device_Monitor_SPK::AMP_CH2, *cntvlu);
    }
    else if (sender() == _ui->gain2Slider)
    {
        _ui->gain2Value->setText(QString::number(i));

        _devAmp->sendVolume(Device_Monitor_SPK::AMP_CH2, i);
    }
    ...
    else if(sender() == _ui->delay3Slider)
    {
        ...
        cntvlu = &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH3].ControlValue;
        cntvlu->delay = i * 3;

        _devAmp->sendCommonDataPack(Device_Monitor_SPK::AMP_CH3, *cntvlu);
    }
    else if (sender() == _ui->gain3Slider)
    {
        _ui->gain3Value->setText(QString::number(i));

        _devAmp->sendVolume(Device_Monitor_SPK::AMP_CH3, i);
    }
    ...
    else if(sender() == _ui->delay4Slider)
    {
        ...
        cntvlu = &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH4].ControlValue;
        cntvlu->delay = i * 3;

        _devAmp->sendCommonDataPack(Device_Monitor_SPK::AMP_CH4, *cntvlu);
    }
    else if (sender() == _ui->gain4Slider)
    {
        _ui->gain4Value->setText(QString::number(i));

        _devAmp->sendVolume(Device_Monitor_SPK::AMP_CH4, i);
    }
}

void DevSetting_Monitor_SPK::setSliderValue()
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
 ...
 *
 *****************************************************************************************/
void DevSetting_Monitor_SPK::on_router_clicked()
{
 ...
 *****************************************************************************************/
/* 第一路 */
void DevSetting_Monitor_SPK::on_eq1_clicked()
{
//    EqData8B& eq4B = _devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH1].Eq;//.ch1_Eq;
//    QDialog dlg;
//    EqView_AllBand *eqView = new EqView_AllBand(8, &dlg);
//    eqView->showWidget(Device_Monitor_SPK::AMP_CH1, QString(), eq4B.bypass, eq4B.eq);
...
/* 第二路 */
void DevSetting_Monitor_SPK::on_eq2_clicked()
{
//    EqData8B& eq4B = _devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH2].Eq;//.ch2_Eq;
//    QDialog dlg;
//    EqView_AllBand *eqView = new EqView_AllBand(8, &dlg);
//    eqView->showWidget(Device_Monitor_SPK::AMP_CH2, QString(), eq4B.bypass, eq4B.eq);
//    dlg.resize(eqView->size());
//    connect(eqView, SIGNAL(eqDataChanged(quint8, bool, EqData*)), this, SLOT(onEqChanged(quint8, bool, EqData*)));
//    dlg.exec();
}
...
/* 第三路 */
void DevSetting_Monitor_SPK::on_eq3_clicked()
{
    EqData8B& eq4B = _devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH3].Eq;
    DevSettingDlg dlg;
    EqView_AllBand *eqView = new EqView_AllBand(8, &dlg);
    eqView->showWidget(Device_Monitor_SPK::AMP_CH3, QString(), eq4B.bypass, eq4B.eq);
    dlg.setWindowTitle(tr("MS660i"));
    dlg.resize(eqView->size());
    connect(eqView, SIGNAL(eqDataChanged(quint8, bool, EqData*)), this, SLOT(onEqChanged(quint8, bool, EqData*)));
    dlg.exec();
}
...
/* 第四路 */
void DevSetting_Monitor_SPK::on_eq4_clicked()
{
    EqData8B& eq4B = _devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH4].Eq;
    DevSettingDlg dlg;
    EqView_AllBand *eqView = new EqView_AllBand(8, &dlg);
    eqView->showWidget(Device_Monitor_SPK::AMP_CH4, QString(), eq4B.bypass, eq4B.eq);
...
/* 处理函数 */
void DevSetting_Monitor_SPK::onEqChanged(quint8 ch, bool bypass, EqData *eqPara)
{
 ...
 *****************************************************************************************/
/* 第一路 */
void DevSetting_Monitor_SPK::on_filter1HPF_clicked()
{
    _ui->filter1HPFSet->setEnabled(true);
    _ui->filter1LPFSet->setEnabled(false);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH1, false, true);
}

void DevSetting_Monitor_SPK::on_filter1Bypass_clicked()
{
    _ui->filter1HPFSet->setEnabled(false);
    _ui->filter1LPFSet->setEnabled(false);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH1, true, true);
}

void DevSetting_Monitor_SPK::on_filter1LPF_clicked()
{
    _ui->filter1HPFSet->setEnabled(false);
    _ui->filter1LPFSet->setEnabled(true);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH1, true, false);
}

void DevSetting_Monitor_SPK::on_filter1HPFSet_clicked()
{
    AmpFilter1 pf(Device_Monitor_SPK::AMP_CH1, &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH1].divider.hf);
    pf.setWindowTitle(tr("Channel1 HF"));
    connect(&pf, SIGNAL(pfDataChanged(quint8)), this, SLOT(onPfDataChanged(quint8)));
    pf.exec();
}

void DevSetting_Monitor_SPK::on_filter1LPFSet_clicked()
{
    AmpFilter1 pf(Device_Monitor_SPK::AMP_CH1, &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH1].divider.lf);
...
/* 第二路 */
void DevSetting_Monitor_SPK::on_filter2HPF_clicked()
{
    _ui->filter2HPFSet->setEnabled(true);
    _ui->filter2LPFSet->setEnabled(false);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH2, false, true);
}

void DevSetting_Monitor_SPK::on_filter2Bypass_clicked()
{
    _ui->filter2HPFSet->setEnabled(false);
    _ui->filter2LPFSet->setEnabled(false);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH2, true, true);
}

void DevSetting_Monitor_SPK::on_filter2LPF_clicked()
{
    _ui->filter2HPFSet->setEnabled(false);
    _ui->filter2LPFSet->setEnabled(true);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH2, true, false);
}

void DevSetting_Monitor_SPK::on_filter2HPFSet_clicked()
{
    AmpFilter1 pf(Device_Monitor_SPK::AMP_CH2, &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH2].divider.hf);
    pf.setWindowTitle(tr("Channel2 HF"));
    connect(&pf, SIGNAL(pfDataChanged(quint8)), this, SLOT(onPfDataChanged(quint8)));
    pf.exec();
}

void DevSetting_Monitor_SPK::on_filter2LPFSet_clicked()
{
    AmpFilter1 pf(Device_Monitor_SPK::AMP_CH2, &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH2].divider.lf);
...
/* 第三路 */
void DevSetting_Monitor_SPK::on_filter3HPF_clicked()
{
    _ui->filter3HPFSet->setEnabled(true);
    _ui->filter3LPFSet->setEnabled(false);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH3, false, true);
}

void DevSetting_Monitor_SPK::on_filter3Bypass_clicked()
{
    _ui->filter3HPFSet->setEnabled(false);
    _ui->filter3LPFSet->setEnabled(false);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH3, true, true);
}

void DevSetting_Monitor_SPK::on_filter3LPF_clicked()
{
    _ui->filter3HPFSet->setEnabled(false);
    _ui->filter3LPFSet->setEnabled(true);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH3, true, false);
}

void DevSetting_Monitor_SPK::on_filter3HPFSet_clicked()
{
    AmpFilter1 pf(Device_Monitor_SPK::AMP_CH3, &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH3].divider.hf);
    pf.setWindowTitle(tr("Channel3 HF"));
    connect(&pf, SIGNAL(pfDataChanged(quint8)), this, SLOT(onPfDataChanged(quint8)));
    pf.exec();
}

void DevSetting_Monitor_SPK::on_filter3LPFSet_clicked()
{
    AmpFilter1 pf(Device_Monitor_SPK::AMP_CH3, &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH3].divider.lf);
...
/* 第四路 */
void DevSetting_Monitor_SPK::on_filter4HPF_clicked()
{
    _ui->filter4HPFSet->setEnabled(true);
    _ui->filter4LPFSet->setEnabled(false);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH4, false, true);
}

void DevSetting_Monitor_SPK::on_filter4Bypass_clicked()
{
    _ui->filter4HPFSet->setEnabled(false);
    _ui->filter4LPFSet->setEnabled(false);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH4, true, true);
}

void DevSetting_Monitor_SPK::on_filter4LPF_clicked()
{
    _ui->filter4HPFSet->setEnabled(false);
    _ui->filter4LPFSet->setEnabled(true);

    onPfDataChanged(Device_Monitor_SPK::AMP_CH4, true, false);
}

void DevSetting_Monitor_SPK::on_filter4HPFSet_clicked()
{
    AmpFilter1 pf(Device_Monitor_SPK::AMP_CH4, &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH4].divider.hf);
    pf.setWindowTitle(tr("Channel4 HF"));
    connect(&pf, SIGNAL(pfDataChanged(quint8)), this, SLOT(onPfDataChanged(quint8)));
    pf.exec();
}

void DevSetting_Monitor_SPK::on_filter4LPFSet_clicked()
{
    AmpFilter1 pf(Device_Monitor_SPK::AMP_CH4, &_devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH4].divider.lf);
...
/* 处理函数 */
void DevSetting_Monitor_SPK::onPfDataChanged(quint8 channel)
{
...
void DevSetting_Monitor_SPK::onPfDataChanged(quint8 ch, bool hfb, bool lfb)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
 ...
 *****************************************************************************************/
/* 第一路 */
void DevSetting_Monitor_SPK::on_dyn1_clicked()
{
    DynData& dynData = _devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH1].Dyn;//.ch1_Dyn;
    DevSettingDlg dlg;
    Trn_DynWidget *dynWidget = new Trn_DynWidget(&dlg);
    dynWidget->setDisabled(_devAmp->isFactoryMode());
    dynWidget->showWidget(false, Device_Monitor_SPK::AMP_CH1, QString(), dynData);
...
/* 第二路 */
void DevSetting_Monitor_SPK::on_dyn2_clicked()
{
    DynData& dynData = _devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH2].Dyn;//.ch2_Dyn;
    DevSettingDlg dlg;
    Trn_DynWidget *dynWidget = new Trn_DynWidget(&dlg);
    dynWidget->setDisabled(_devAmp->isFactoryMode());
    dynWidget->showWidget(false, Device_Monitor_SPK::AMP_CH2, QString(), dynData);
    dlg.resize(dynWidget->size());
    connect(dynWidget, SIGNAL(dynChanged(quint8,bool,DynData)), this, SLOT(onDynChanged(quint8,bool,DynData)));
    dlg.exec();
}
...
/* 第三路 */
void DevSetting_Monitor_SPK::on_dyn3_clicked()
{
    DynData& dynData = _devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH3].Dyn;
    DevSettingDlg dlg;
    Trn_DynWidget *dynWidget = new Trn_DynWidget(&dlg);
    dynWidget->setDisabled(_devAmp->isFactoryMode());
    dynWidget->showWidget(false, Device_Monitor_SPK::AMP_CH3, QString(), dynData);
    dlg.resize(dynWidget->size());
    connect(dynWidget, SIGNAL(dynChanged(quint8,bool,DynData)), this, SLOT(onDynChanged(quint8,bool,DynData)));
    dlg.exec();
}
...
/* 第四路 */
void DevSetting_Monitor_SPK::on_dyn4_clicked()
{
    DynData& dynData = _devAmp->getConfigData().CH[Device_Monitor_SPK::AMP_CH4].Dyn;
    DevSettingDlg dlg;
    Trn_DynWidget *dynWidget = new Trn_DynWidget(&dlg);
    dynWidget->setDisabled(_devAmp->isFactoryMode());
    dynWidget->showWidget(false, Device_Monitor_SPK::AMP_CH4, QString(), dynData);
    dlg.resize(dynWidget->size());
    connect(dynWidget, SIGNAL(dynChanged(quint8,bool,DynData)), this, SLOT(onDynChanged(quint8,bool,DynData)));
    dlg.exec();
}
...
/* 处理函数 */
void DevSetting_Monitor_SPK::onDynChanged(quint8 ch, bool isIn ,DynData dynData)
{
    Q_UNUSED(isIn)
 ...
 *
 *****************************************************************************************/
void DevSetting_Monitor_SPK::loadAlarmSetting()
{
    AlarmConfig *alarmCfg = _devAmp->getAlarmCfg();
...
void DevSetting_Monitor_SPK::saveAlarmSetting()
{
   AlarmConfig *alarmCfg = _devAmp->getAlarmCfg();
