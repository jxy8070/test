...
// 构造函数
DevSetting_Spk::DevSetting_Spk(DeviceItem_Spk *devItem, QWidget *parent)
    ...
    _ui(new Ui::DevSetting_Spk)
{
    ...
    _devSpk =qobject_cast<Device_Spk*>(devItem->getAES70Device());
    ...
    _ui->Mac_addr->setText(_devSpk->getAES70Mac());
...
void DevSetting_Spk::keyPressEvent(QKeyEvent *e)
{
  switch (e->key()) {
  case Qt::Key_Enter:
      break;
...
void DevSetting_Spk::factoryModeReInit()
{
...
// 显示设备发来的电压电流温度等数据
void DevSetting_Spk::onAdcDataChanged(AlarmConfig *alarmCfg)
{
...
void DevSetting_Spk::connectSigSlots()
{
    /* 第一路延时选择 */
...
void DevSetting_Spk::setAlarmMode(bool b)
{
    _ui->groupBox->setEnabled(b);
    _devSpk->getAlarmCfg()->setAlarmEnable(b);
}

void DevSetting_Spk::on_router_clicked()
{
    SpkRouter router(_devSpk, this);
    router.exec();
}

void DevSetting_Spk::on_alarmButton_clicked()
{
    AlarmRecord alarmFrm(_devSpk->getAES70Mac());
    alarmFrm.exec();
}

void DevSetting_Spk::on_eq_clicked()
{
...
void DevSetting_Spk::on_dividerHF_clicked()
{
...
void DevSetting_Spk::on_dividerLF_clicked()
{
...
void DevSetting_Spk::onPfDataChanged(quint8 channel)
{
    Q_ASSERT(channel == 1);
...
void DevSetting_Spk::onPfDataChanged(quint8 ch, bool hfb, bool lfb)
{
    Q_ASSERT(ch == 1);
...
void DevSetting_Spk::on_dyn_clicked()
{
...
void DevSetting_Spk::onEqChanged(quint8 ch, bool bypass, EqData *eqPara)
{
    _devSpk->sendEqPack(ch, bypass, eqPara);
}

void DevSetting_Spk::onDynChanged(quint8 ch, bool isIn ,DynData dynData)
{
    Q_UNUSED(isIn)
    _devSpk->sendDynPack(ch, dynData);
}

void DevSetting_Spk::setLimiterValue()
{
...
void DevSetting_Spk::setIntValueDisplay(int i)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
...
void DevSetting_Spk::setEnableWidgets(int i)
{
    AlarmConfig *alarmCfg = _devSpk->getAlarmCfg();
...
void DevSetting_Spk::setSliderValue()
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
...
// 保存到设备
void DevSetting_Spk::save2Device()
{
    _devSpk->sendSave2DevicePack();
}

void DevSetting_Spk::displayConfigData()
{
    _syncing = true;
...
// 加载场景文件
void DevSetting_Spk::on_loadButton_clicked()
{
...
// 另存场景为文件
void DevSetting_Spk::on_saveAsButton_clicked()
{
...
void DevSetting_Spk::on_pushButton60v_clicked()
{
    resetVoltCurrLimit(0);
    _devSpk->sendVoltTypePack(0);
}

void DevSetting_Spk::on_pushButton100v_clicked()
{
...
void DevSetting_Spk::resetVoltCurrLimit(quint8 voltType)
{
...
void DevSetting_Spk::loadAlarmSetting()
{
    AlarmConfig *alarmCfg = _devSpk->getAlarmCfg();
...
void DevSetting_Spk::saveAlarmSetting()
{
   AlarmConfig *alarmCfg = _devSpk->getAlarmCfg();
   alarmCfg->saveAlarmSetting();
}

void DevSetting_Spk::on_gain1P_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
...
void DevSetting_Spk::on_gain1M_clicked(bool checked)
{
    /* 数据同步时不再次发送 */
    if (_syncing)
    {
