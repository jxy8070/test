...
typedef struct dante_domain_info
{
	...
	dante_capability_mask_t _internal;
		// For internal use only, do not use directly.
		// May be hidden / altered / removed in future
} dante_domain_info_t;
...
typedef enum ddh_state
{
	...
	DDH_STATE_CONNECTING,   //!< The domain handler is connecting to a Dante Domain Manager
 ...
 *
 * @pre handler is in the DISABLED or DISCOVERED state
 ...
 *
 * @pre handler is in the DISABLED, DISCOVERING or DISCOVERED state
 * @post On success, handler is in the DISCOVERED state
 */
aud_error_t
...
/**
 * Initiate a connection to the Domain Manager
 * @pre handler is in the DISCOVERED state
 ...
 * @pre handler is in the CONNECTING or CONNECTED state
...
/**
 * Get DDM ip address and port when in CONNECTED state
 * @pre handler is in the CONNECTED/AUTHENTICATED state.
 * 
 */
aud_error_t
