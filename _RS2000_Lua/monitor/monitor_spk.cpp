...
Monitor_Spk::Monitor_Spk(Device_Spk *devSpk, DeviceItem *devItem, QWidget *parent) :
    QGroupBox(parent),
...
//    connect(_devSpk, SIGNAL(adcDataChanged(AlarmConfig*)), this, SLOT(onAdcDataChanged(AlarmConfig*)));
    ...
    //将行编辑器设置为只接受验证器将接受的输入（验证器接受行编辑器的最小值到最大值）
...
Monitor_Spk::~Monitor_Spk()
{
    delete ui;
}

void Monitor_Spk::unLivingDectect()
{
    if (!_devSpk->isOnline())
    {
        ui->devicenamelabel->setStyleSheet("border: 1px solid #999999;\
...
void Monitor_Spk::showEvent(QShowEvent *)//显示事件
{
...
void Monitor_Spk::hideEvent(QHideEvent *)
{
    _livingTimer->stop();
...
void Monitor_Spk::initData()
{
    ui->devicenamelabel->setText(_devItem->getAES70Device()->getDeviceName()); //显示设备名

    ui->pbPower->setChecked(_devSpk->isPowerOn());//保存按钮是否被选中
    pushButtonSetText(ui->pbPower, _devSpk->isPowerOn());//设置按钮显示的text
...
void Monitor_Spk::syncVolumeChanged()
{
    _syncing = true;
...
void Monitor_Spk::syncMuteChanged()
{
    _syncing = true;
...
void Monitor_Spk::syncPhaseChanged()
{
    _syncing = true;
...
void Monitor_Spk::syncRouterChanged()
{
    ui->InputSource->setText(_devSpk->getRouterInfo());
}

void Monitor_Spk::pushButtonSetText(QPushButton *pb, bool flag)
{
    if (flag)
    {
        pb->setText(tr("On"));
...
void Monitor_Spk::onAdcDataChanged(AlarmConfig *alarmCfg)
{
...
void Monitor_Spk::setIntValueDisplay(int i)
{
    /* 同步的时候不再次发送数据 */
    if (_syncing)
    {
...
void Monitor_Spk::setSliderValue()
{
    /* 同步的时候不再次发送数据 */
    if (_syncing)
    {
...
void Monitor_Spk::processPbClicked(bool flag)
{
