...
#include <QWidget>
#include "Lua/Packages/luaExtension.h"
...
luaQ_setup(lua_State *L, QtLuaEngine::Private *d)
{
  ...
  luaL_dostring(L, "require 'printx'");
}
...
void QtLuaEngine::pushFunction(QString funcName, lua_CFunction func)
{
...
find_error_location(lua_State *L, QByteArray &message)
{
  ...
  message = QByteArray(mStart); //设置行号开始的错误消息
...
{
  QVariant *vp = 0;
  bool b1 = lua_isuserdata(L, index);
  bool b2 = lua_getmetatable(L, index);
...
//            || type == QMetaType::QWidgetStar//暂不支持QWidget
