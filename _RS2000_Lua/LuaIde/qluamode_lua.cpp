...
PNode::PNode(TokenTypeLua t, int p, int l, PNode n)
  : QSharedDataPointer<Node>(new Node(t,p,l,n)) {}

PNode::PNode(TokenTypeLua t, int p, int l, int i, PNode n)
  : QSharedDataPointer<Node>(new Node(t,p,l,i,n)) {}

inline TokenTypeLua PNode::type() const {
...
public:
  ...
  void gotToken(UserData *d, int pos, int len, QString, TokenTypeLua);
...
private:
  QMap<QString,TokenTypeLua> keywords;
...
#if DEBUG_LUA
QDebug operator<<(QDebug d, const TokenTypeLua &t)
{
  d.nospace();
...
QLuaModeLua::gotToken(UserData *d, int pos, int len, 
                      QString s, TokenTypeLua ltype)
{
  PNode &n = d->nodes;
  TokenTypeLua ntype = n.type();
#if DEBUG_LUA
...
comp_keywords[] = 
  {
    ...
    "true", "until", "while",
    //扩展
    "Controls.Inputs", "Controls.Outputs",
    "Timer", "EventHandler", "CallAfter",
    0
  };
...
QLuaModeLua::computeSymbolCompletions(QString s, QString &stem)
{
  ...
  // symbols
  QtLuaEngine *engine = new QtLuaEngine; //wj 全局唯一
  if (engine)
    {
      ...
      if (lua)
        ...
        {
//          showMessage(tr("Auto-completions is restricted while Lua is running.") );
          QApplication::beep();
        }
    }
  return list;
